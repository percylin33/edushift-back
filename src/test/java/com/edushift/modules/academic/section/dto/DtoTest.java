...
class DtoTest {
    ...
    void sectionResponse() {
        var resp = new SectionResponse(UUID.randomUUID(), UUID.randomUUID(), "2026", "ACTIVE",
            ...
            "A", 30, 1, null, null, Instant.now(), Instant.now());
        assertThat(resp.name()).isEqualTo("A");
        assertThat(resp.capacity()).isEqualTo(30);
        assertThat(resp.teachingMode()).isEqualTo("POLIDOCENTE");
    }
    ...
    void createSectionRequest() {
        var req = new CreateSectionRequest(UUID.randomUUID(), UUID.randomUUID(), "A", 30, 1, null);
    ...
    void updateSectionRequest() {
        var empty = new UpdateSectionRequest(null, null, null, null, null);
        assertThat(empty.isEmpty()).isTrue();

        var nonEmpty = new UpdateSectionRequest("B", null, null, null, null);
        assertThat(nonEmpty.isEmpty()).isFalse();
        assertThat(nonEmpty.name()).isEqualTo("B");

        var clearHomeroom = new UpdateSectionRequest(null, null, null, null, true);
        assertThat(clearHomeroom.isEmpty()).isFalse();
    }
}
