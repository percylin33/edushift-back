...
class AttendanceDtoTest {
    ...
    @Test void recordResponse() { var r = new AttendanceRecordResponse(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), "A", AttendanceRecordStatus.PRESENT, Instant.now(), null, null, null, null, null, Instant.now(), Instant.now(), null, null, null, null); assertThat(r.studentFullName()).isEqualTo("A"); }
